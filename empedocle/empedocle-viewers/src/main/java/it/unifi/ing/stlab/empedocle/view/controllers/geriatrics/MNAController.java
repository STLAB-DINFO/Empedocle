...
@Named
@RequestScoped
public class MNAController extends ContainerController {
	...
	public String retrieveOverallScore( Viewer viewer, Fact value ) {
		...
		QuantitativeFact scoreFact = ClassHelper.cast( facts.remove( 6 ), QuantitativeFact.class );
	...
	public String retrieveScoreClassification( Viewer viewer, Fact value ) {
		try {
			...
			if( score <= 7 )
				...
				return "a rischio di malnutrizione";
			else if ( score <= 14 )
				return "stato nutrizionale normale";
